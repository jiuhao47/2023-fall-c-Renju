...
int chainjudge(int *chessshape) {
  ...
  for (int mode = 0; mode < DIRECTION; mode++) {
    count = 0;
    for (int i = 0; i < 9; i++) {
      if (chessshape[mode * 9 + i] ==
              (gamestates.playerstate != BLACK ? BLACKCHESS : WHITECHESS) ||
          chessshape[mode * 9 + i] == (gamestates.playerstate != BLACK
                                           ? BLACKCHESSCURRENT
                                           : WHITECHESSCURRENT)) {
        count = count + 1;
...
int doublefourjudge(int x, int y, int *chessshape) {
  ...
  for (int mode = 0; mode < DIRECTION; mode++) {
    if ((chainjudge_mode(mode, chessshape) < 4) &&
        (fourjudge(x, y, mode, chessshape) == 2)) {
      return 1;
    }
  ...
  for (int mode = 0; mode < DIRECTION; mode++) {
    ...
    for (int i = -4; i <= 4; i++) {
      ...
      if (pointInBoard(temppoint.x, temppoint.y)) {
        chessshape[mode * 9 + i + 4] =
            Board[displayPosToInnerPos(temppoint.x, temppoint.y)];
      } else {
